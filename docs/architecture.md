...
The App Router serves organizer, join, camera, scorer, and fixed-aspect program pages. Route handlers validate commands with Zod. A process-local repository is the mock authority; clients poll it and use `BroadcastChannel` only as a low-latency refresh hint. Score history is append-only, including compensating Undo events. Sponsor timing derives from the server-stamped `startedAt`, interval, and offset—not a scorer-local index.
...
Mock state is single-process and unsuitable for multi-instance/serverless production. Upload data URLs are deliberately a development convenience, not cloud storage.

## Mock shortcut audit

| Shortcut or risk                                                                    | Classification                               | Required follow-up                                                                           |
| ----------------------------------------------------------------------------------- | -------------------------------------------- | -------------------------------------------------------------------------------------------- |
| Process-memory games reset on restart/serverless isolation                          | Must be replaced during Supabase integration | Durable tables, transactions, RLS, and Realtime                                              |
| Polling/`BroadcastChannel` only synchronizes one process and nearby browser windows | Must be replaced during Supabase integration | Realtime persisted version stream                                                            |
| Role claims are non-atomic and process-local                                        | Must be replaced during Supabase integration | Unique constraint plus transactional invitation consumption                                  |
