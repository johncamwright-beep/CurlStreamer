...
| `supabase-store.save` via PATCH, claim, release, disconnect, deletion revocation, layout/audio/broadcast/sponsor/camera/connection/health, and legacy Close Game | Whole `game_states.state` and `version`                                                 | PostgreSQL locks the target state row before the row trigger. The trigger therefore takes no second row lock: it rejects a terminal locked `OLD` state and also checks persisted completion metadata. A writer already holding the state lock finishes first; completion then clears its claims and media state.       |
...
| `update_scheduled_team_game`                                                                                                                                     | Schedule/config columns on `games`, mirrored config in `game_states`, audit             | The deployed RPC signature is preserved and redefined to lock `game_states` before `games`. Updates advance the result revision, and a reviewed revision conflicts if either authoritative config representation changed.                                                                                              |
| `updateScheduledTeamGame` compatibility snapshot restoration                                                                                                     | Direct server-role `games.config` update followed by a direct whole-state config update | Both writes are independently guarded. If completion falls between the two transactions, the second is rejected; the first advanced the revision before completion validation.                                                                                                                                         |
...
are rejected. This deliberately does not redesign ordinary active-game
