...
match both the participant identity and its assignment generation (or the
organizer-observed pair) captured when the operation began. Reusing the same
device ID after release does not satisfy this check. A release or reassignment
therefore stops the retry instead of letting a heartbeat revive the camera.
Scoring, Undo, and other operator intent are never automatically rebased.
