...
try {
  ...
  & $node node_modules/esbuild/bin/esbuild scripts/m4-operator.ts --bundle --platform=node --format=esm --minify '--banner:js=import { createRequire } from "node:module"; const require = createRequire(import.meta.url);' "--outfile=$SetupRoot/m4-operator.mjs"
