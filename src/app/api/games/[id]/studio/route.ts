...
import { studioRequestSchema, signalAllowed } from "@/lib/studio-protocol";
import {
  ...
  StudioRejected,
} from "@/lib/providers/studio-session";
...
export async function POST(
  ...
  try {
    ...
    if (
      ...
      return response({ error: "Camera 1 claim required" }, 403);
    const authority =
    ...
    return cause instanceof StudioRejected
      ...
      : response(
          {
            error: "M1 requires configured disposable Supabase infrastructure.",
          },
          503,
        );
  }
