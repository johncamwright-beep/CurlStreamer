-- M1 only. Apply to disposable infrastructure, never a shared project during the spike.
-- No SDP, ICE, addresses, signaling history, or provider credentials are stored here.
create sequence public.m1_studio_generation;
create table public.m1_studio_sessions (
  game_id uuid primary key references public.games(id) on delete cascade,
  organization_id uuid not null references public.organizations(id),
  id uuid not null unique,
  generation bigint not null default nextval('public.m1_studio_generation'),
...
alter table public.m1_studio_sessions enable row level security;
revoke all on public.m1_studio_sessions, public.m1_studio_generation from public, anon, authenticated;
...
create function public.m1_studio_action(
  p_game_id uuid, p_action text, p_side text,
...
declare
  ...
  v_session public.m1_studio_sessions%rowtype;
  v_generation bigint;
begin
  if p_side not in ('receiver','camera') or p_action not in ('register','begin','ticket','check','stop','signal')
  ...
  v_generation:=coalesce((v_state#>>'{claimGenerations,camera-home}')::bigint,0);
  if p_side='camera' and (p_device_id is null or p_assignment_generation is null
    or (v_state#>>'{claims,camera-home}') is distinct from p_device_id::text
    or v_generation <> p_assignment_generation) then
    raise exception 'camera_released' using errcode='42501';
  end if;
  if p_action='register' then
    insert into public.m1_studio_sessions(game_id,organization_id,id,status)
    values(p_game_id,v_game.organization_id,gen_random_uuid(),'active')
    on conflict(game_id) do update set id=gen_random_uuid(),generation=nextval('public.m1_studio_generation'),
    ...
    select * into v_session from public.m1_studio_sessions where game_id=p_game_id for update;
    ...
    if p_action='begin' then
      update public.m1_studio_sessions set negotiation_id=gen_random_uuid(),
        assignment_generation=v_generation,camera_device_id=p_device_id,camera_seen_at=now(),
        signal_window_at=now(),signal_count=0 where game_id=p_game_id returning * into v_session;
    elsif p_action='stop' then
      update public.m1_studio_sessions set status='stopped' where game_id=p_game_id returning * into v_session;
    else
      ...
      if p_action <> 'check' or p_negotiation_id is not null or p_side='camera' then
        if v_session.negotiation_id is null or v_session.assignment_generation is distinct from v_generation
          or (v_state#>>'{claims,camera-home}') is distinct from v_session.camera_device_id::text
      ...
      update public.m1_studio_sessions set
        ...
        where game_id=p_game_id returning * into v_session;
    end if;
  end if;
  return jsonb_build_object('sessionId',v_session.id,'generation',v_session.generation,
...
$$;
revoke all on function public.m1_studio_action(uuid,text,text,uuid,uuid,uuid,bigint,uuid) from public,anon,authenticated,service_role;
grant execute on function public.m1_studio_action(uuid,text,text,uuid,uuid,uuid,bigint,uuid) to service_role;

create function public.m1_can_receive(p_topic text) returns boolean
language sql stable security definer set search_path='' as $$
  select exists (
    select 1 from public.m1_studio_sessions s
    ...
    where s.status='active' and s.expires_at>now()
      ...
      and (gs.state#>>'{claims,camera-home}')=s.camera_device_id::text
      and coalesce((gs.state#>>'{claimGenerations,camera-home}')::bigint,0)=s.assignment_generation
      and (auth.jwt()->>'m1_session')=s.id::text
      and (auth.jwt()->>'m1_game')=s.game_id::text
      and (auth.jwt()->>'m1_generation')=s.generation::text
      and (auth.jwt()->>'m1_assignment')=s.assignment_generation::text
      and (auth.jwt()->>'m1_negotiation')=s.negotiation_id::text
      and (auth.jwt()->>'m1_side') in ('camera','receiver')
      and (auth.jwt()->>'exp')::numeric>extract(epoch from now())
      and p_topic='m1:'||s.id::text||':'||s.generation::text||':'||s.negotiation_id::text||':'||(auth.jwt()->>'m1_side')
      and (auth.jwt()->>'m1_topic')=p_topic
  );
$$;
revoke all on function public.m1_can_receive(text) from public,anon,authenticated;
grant execute on function public.m1_can_receive(text) to authenticated;
create policy m1_receive on realtime.messages for select to authenticated
  using (extension='broadcast' and public.m1_can_receive(realtime.topic()));
-- Restrictive policies keep broad pre-existing policies from granting M1 access.
create policy m1_receive_fence on realtime.messages as restrictive for select to authenticated
  using (case when realtime.topic() like 'm1:%' then extension='broadcast' and public.m1_can_receive(realtime.topic())
    else coalesce(auth.jwt()->>'m1_session','')='' end);
create policy m1_no_client_send on realtime.messages as restrictive for insert to authenticated
  with check (realtime.topic() not like 'm1:%' and coalesce(auth.jwt()->>'m1_session','')='');
