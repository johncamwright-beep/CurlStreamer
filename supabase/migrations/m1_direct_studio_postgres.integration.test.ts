...
function action(
  ...
  return (
    "select public.m1_studio_action(" +
    [
      quote(game),
      quote(operation),
      quote(side),
...
describe.skipIf(!enabled)("M1 disposable PostgreSQL authority", () => {
  ...
  it("expires receiver heartbeats and prevents resurrection", () => {
    ...
    expect(
      sql(
        "update public.m1_studio_sessions set receiver_seen_at=now()-interval '31 seconds' where game_id='" +
          game +
          "'",
      ).ok,
    ).toBe(true);
    ...
    const canRead = (overrides: object = {}, target = topic) =>
      sql(
        "set role authenticated; set request.jwt.claims='" +
          ...
          "'; select public.m1_can_receive('" +
          target +
          "')",
      );
    expect(canRead().stdout).toBe("t");
    expect(canRead({ exp: 0 }).stdout).toBe("f");
    expect(canRead({ m1_generation: 99 }).stdout).toBe("f");
  ...
  it("keeps restrictive Realtime policies effective even with permissive policies", () => {
    ...
    const checks = sql(
      "begin; create policy m1_test_broad on realtime.messages for all to authenticated using(true) with check(true); insert into realtime.messages(extension,topic) values('broadcast','" +
        topic +
    ...
    const send = sql(
      "begin; create policy m1_test_broad on realtime.messages for all to authenticated using(true) with check(true); set role authenticated; set realtime.topic='" +
        topic +
  ...
  it("completion holds the lock before a concurrent signaling write", async () => {
    ...
    const complete = held(
      "select * from public.complete_reviewed_game('" +
        ...
        randomUUID() +
        "',null,true)",
      "m1_complete_" + randomUUID(),
    );
    await complete.ready;
    const app = "m1_signal_" + randomUUID();
